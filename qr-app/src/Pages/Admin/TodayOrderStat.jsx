...
export const TodayOrderStat = () => {
    ...
    console.log(items)
    return (
        <div className='h-screen ml-2 w-[100%]'>
            <ReverseButton route={'/admin'} routeName={'Admin'} css={'mt-3'} />
            <div className="p-4">
                <h1 className="text-2xl font-bold mb-6">Today's Orders</h1>

                {Object.entries(items).map(([status, orders]) => (
                    <div key={status} className="mb-10">
                        <h2 className={`text-xl font-semibold ${status === 'delivered' ? 'bg-green-400' : 'bg-red-400'} capitalize mb-4 text-white bg-blue-500 px-4 py-1 rounded-lg inline-block`}>
                                ...
                                <tbody>
                                    {orders.map((order, index) => (
                                        <tr key={order._id} className="hover:bg-gray-50">
                                            ...
                                            <td className="py-2 px-4 border-b">...{order._id.slice(-5)}</td>
                                            <td className="py-2 px-4 border-b">{order.userId?.name}</td>
                                            <td className="py-2 px-4 border-b">{order.userId?.phone}</td>
                                            ...
                                            <td className="py-2 px-4 border-b">
                                                <ul className="list-disc pl-4 text-sm text-gray-700">
                                                    {order.items?.map((item, idx) => (
                                                        <li key={idx}>
                                                            {item.productId.name} — ₹{item.price} × {item.quantity}
                                                        </li>
                                                    ))}
                                                </ul>
                                            </td>
