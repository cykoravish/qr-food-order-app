            ...
            {user && Array.isArray(orders)
                ? <div className="space-y-4">
                    {orders.map((order, index) => (
                        <details key={order._id} className="border border-gray-300 rounded-xl p-4 gap-4 shadow-md bg-white transition-all">
                            <summary className=" min-w-[280px] cursor-pointer font-semibold text-md text-blue-600 flex flex-col gap-4 justify-between items-center" >
                                <span className='flex '>{index + 1}-OrderId: {order._id.slice(-5)}<span className='ml-5 mx-auto rounded-md '><MdExpandCircleDown size={25} /></span></span>
                                <span className="text-sm text-gray-500">{new Date(order.placedAt).toLocaleString()}</span>
                            </summary>

                            <div className="mt-4 space-y-2">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                    <p><span className="font-medium">Customer:</span> {order.userId?.name}</p>
                                    ...
                                    <p><span className="font-medium">Tax:</span> ₹{order.tax}</p>
                                    <p><span className="font-medium">Delivery Charge:</span> ₹{order.deliveryCharge}</p>
                                </div>
                                ...
                                <div className="mt-4">
                                    ...
                                    <button className='ml-2 text-blue-500' onClick={() => window.print()}>Print</button>
                                </div>
                            </div>
                        </details>
                    ))}
                ...
                : (orders &&
                    <div key={orders._id} className="border rounded-lg p-4 mb-4 shadow-md">
                        <h2 className="font-medium   mb-2">Order ID: {orders._id}</h2>
                        <p><span className="font-medium">Customer:</span> {orders.userId?.name}</p>
                        <p><span className="font-medium">Phone:</span> {orders.userId?.phone}</p>
                        <p><span className="font-medium">Status:</span> {orders.status}</p>
                        <p><span className="font-medium">Payment:</span> {orders.paymentMethod}</p>
                        <p><span className="font-medium">Total:</span> ₹{orders.totalAmount}</p>
                        <p><span className="font-medium">Tax:</span> ₹{orders.tax}</p>
                        <p><span className="font-medium">Delivery Charge:</span> ₹{orders.deliveryCharge}</p>
                        <p><span className="font-medium">Placed At:</span> {new Date(orders.placedAt).toLocaleString()}</p>
